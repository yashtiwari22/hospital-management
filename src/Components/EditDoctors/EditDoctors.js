...
function EditDoctors() {
    ...
    const onSubmit = async e => {
        e.preventDefault();
        await axios.put(`http://localhost:9001/Doctors/${id}`, doctor);
        history.push("/admin");
        console.log(doctor);
    }
    const loadDoctor = async () => {
        const result = await axios.get(`http://localhost:9001/Doctors/${id}`);
    ...
    return (<>
        ...
        >
            ...
            <div className="datade">
                ...
                <TextField style={{margin:"15px"}} label="Specialization" variant="outlined" name="specialization" value={specialization} onChange={e => onInputChange(e)} />
                <Button style={{margin:"20px"}} variant="contained" onClick={e => onSubmit(e)}>Update</Button>
                </div>
            </Paper>
        </Box>
    </>
